	...
	for(int i = 0 ; i < FMDEPTH ; i++){ 
		for(int j = 0 ; j < (FMGSIZE) ; j++){
			for(int k = 0 ; k < (FMGSIZE) ; k++){ 
				inNeuIdx = i*(FMGSIZE)*(FMGSIZE) + j*(FMGSIZE) + k;
				inNeuIdy = i*(FMSIZE)*(FMSIZE) + (j-5/2)*FMSIZE + (k-5/2);
				if(j >= 2 && j <= 28 && k >=2  && k <= 28) tmp = inNeu[inNeuIdy];
				else  tmp = 0 ;				
				inGNeu[inNeuIdx] = tmp;
			}
		}
